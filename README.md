...
🔗 **Live Demo:** https://auctionsx.tech/
...
* Sold announcements spoken aloud via the Web Speech API
...
Want to try it without setting anything up first?
...
3. Set the environment variables:
   ...
   * `VITE_WS_URL` — your deployed backend WebSocket endpoint
4. Deploy — Vite builds the application with `npm run build` and serves the `dist/` output
...
```powershell
git add README.md
git commit -m "Update production live demo URL"
git push
````

After pushing, your GitHub README will point users to **`https://auctionsx.tech/`** instead of the old Vercel URL.
